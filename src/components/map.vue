<template>
  <div>
    ...
    <div id="r-result">请输入:<input type="text" id="suggestId" size="20"  name="address_detail"  v-model="address_detail" style="width:150px;" /></div>
    ...
    export default {
      ...
      methods:{
        ...
        Zmap(){
          ...
          var point = new BMap.Point(121.489, 31.227263);  // 创建点坐标
          this.map.centerAndZoom(point, 13);                 // 初始化地图，设置中心点坐标和地图级别

          var marker = new BMap.Marker(new BMap.Point(121.489, 31.227263));        // 创建标注
          this.map.addOverlay(marker);
          this.map.enableScrollWheelZoom(true);


          for(let i = 0;i< zmap.mapdetail.length;i++){
              var marker = new BMap.Marker(new BMap.Point(zmap.mapdetail[i].cen_loc.lng, zmap.mapdetail[i].cen_loc.lat));        // 创建标注
              this.map.addOverlay(marker);
            var opts = {
              position : new BMap.Point(zmap.mapdetail[i].cen_loc.lng,zmap.mapdetail[i].cen_loc.lat),    // 指定文本标注所在的地理位置
              offset   : new BMap.Size(-50, 0)    //设置文本偏移量
            }
            var label = new BMap.Label(zmap.mapdetail[i].area_id+ " 等级:" +zmap.mapdetail[i].level+ " 系数:" + zmap.mapdetail[i].area_rate, opts);  // 创建文本标注对象
            label.setStyle({
              color : "red",
              fontSize : "12px",
              height : "20px",
          ...
          function setPlace() {
            ...
            function myFun() {
              ...
              var myIcon = new BMap.Icon(require("../assets/images/marker.png"), new BMap.Size(22, 32));
              zmap.map.addOverlay(new BMap.Marker(zmap.userlocation,{icon:myIcon}));    //添加标注
            }

            var local = new BMap.LocalSearch(zmap.map, { //智能搜索
        ...
        initGrid: function () {//初始化网格
          ...
          for (var i = zMap.bounds.y1 + (zMap.initCenter.lat - zMap.bounds.y1) % span.y - span.y; i < zMap.bounds.y2 + span.y; i += span.y) {
            var polyline = new BMap.Polyline([
        ...
        getGrid: function (point) {//返回当前点在所在区块的四个顶点
          ...
          var marker = new BMap.Marker(new BMap.Point(xpoints[0]+0.0262, ypoints[0]+0.02246));        // 创建标注
          this.map.addOverlay(marker);
        ...
        fromtext(){
          ...
          this.$axios.post("map/map_data",params).then(res=>{
            ...
            var opts = {
              position : new BMap.Point(that.mapsd.cen_loc.lng,that.mapsd.cen_loc.lat),    // 指定文本标注所在的地理位置
              offset   : new BMap.Size(-50, 0)    //设置文本偏移量
            }
            var label = new BMap.Label(that.mapsd.area_id+ " 等级" + that.mapsd.level+ " 系数:"+that.mapsd.area_rate , opts);  // 创建文本标注对象
            label.setStyle({
              color : "red",
              fontSize : "12px",
              height : "20px",
